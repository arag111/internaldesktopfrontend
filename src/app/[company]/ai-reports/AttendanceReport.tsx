...
export default function AttendanceReport() {
  ...
  const handleSendEmail = async (userId: number, email: string) => {
    try {
      ...
      setToastMessage({ message: `Attendance report sent to ${email}`, type: 'success' });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {/* Period Filter - Material Design Chips */}
      ...
      {!loading && !error && attendanceData && (
        <div className="space-y-6">
          ...
          {/* Summary Stats - Material Design Cards */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            <div className="bg-white rounded-lg border border-slate-200 p-6">
              <div className="flex items-center gap-2 mb-3">
                <BarChart3 className="w-5 h-5 text-blue-600" />
                <span className="text-sm font-medium text-slate-600 uppercase tracking-wide">Total Records</span>
              </div>
              <p className="text-3xl font-normal text-slate-900">{attendanceData.totalRecords}</p>
            </div>
            <div className="bg-white rounded-lg border border-slate-200 p-6">
              <div className="flex items-center gap-2 mb-3">
                <CheckCircle className="w-5 h-5 text-green-600" />
                <span className="text-sm font-medium text-slate-600 uppercase tracking-wide">On Time / Early</span>
              </div>
            ...
            <div className="bg-white rounded-lg border border-slate-200 p-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-medium text-slate-600 uppercase tracking-wide">Late</span>
              </div>
              <p className="text-3xl font-normal text-slate-900">
            ...
            <div className="bg-white rounded-lg border border-slate-200 p-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-medium text-slate-600 uppercase tracking-wide">Office Days</span>
              </div>
              <p className="text-3xl font-normal text-slate-900">
                  ...
                  <thead className="bg-slate-50 text-slate-900">
                    <tr>
                      ...
                      <th className="sticky left-0 z-10 bg-slate-50 px-6 py-4 text-left border-r border-slate-200">
                        <div className="text-sm font-medium text-slate-700 uppercase tracking-wide flex items-center gap-2">
                      ...
                      <th className="px-4 py-4 text-center border-r border-slate-200">
                        <div className="text-sm font-medium text-slate-700 uppercase">Days</div>
                      </th>
                      <th className="px-4 py-4 text-center border-r border-slate-200">
                        <div className="text-sm font-medium text-slate-700 uppercase">On-Time</div>
                      </th>
                      <th className="px-4 py-4 text-center border-r border-slate-200">
                        <div className="text-sm font-medium text-slate-700 uppercase">Late</div>
                      </th>
                      <th className="px-4 py-4 text-center border-r-2 border-slate-300">
                        <div className="text-sm font-medium text-slate-700 uppercase">Office</div>
                      </th>
                      ...
                      <th className="px-4 py-4 text-center">
                        <div className="text-sm font-medium text-slate-700 uppercase">Action</div>
                      </th>
                    </tr>
                  </thead>
                        ...
                        <tr key={userAttendance.user.id} className="hover:bg-slate-50 transition-colors duration-200 border-b border-slate-200">
                          ...
                          <td className="sticky left-0 z-10 bg-white px-6 py-4 border-r border-slate-200">
                            <div className="min-w-[200px]">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <div className="font-medium text-slate-900">{userAttendance.user.name}</div>
                              </div>
                              <div className="text-sm text-slate-600">{userAttendance.user.jobRole}</div>
                                ...
                                {record ? (
                                  <div className="space-y-2 p-3 bg-slate-50 rounded-lg">
                                    <div className="flex items-center gap-2 text-sm">
                                      ...
                                      <span className="font-medium text-slate-900">{record.punchInTimeFormatted}</span>
                                    </div>
                                    <div className="flex gap-2 flex-wrap">
                                      <span className={`px-2.5 py-1 text-xs font-medium rounded-md ${getStatusBadgeClass(record.statusColor)}`}>
                                        {record.status}
                                      </span>
                                      <span className={`px-2.5 py-1 text-xs font-medium rounded-md ${getLocationBadgeClass(record.locationColor)}`}>
                                        {record.location}
                                      </span>
                                    </div>
                                  ...
                                  <div className="text-center text-slate-400 text-sm py-2">-</div>
                                )}
                              </td>
                            );
                          })}
                          ...
                          <td className="px-4 py-4 text-center">
                            <button
                              ...
                              className="inline-flex items-center gap-2 px-4 py-2 text-blue-600 text-sm font-medium hover:bg-blue-50 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
                            >
